...
def extract_tests_from_source(lines):
    ...
    match_url = re.compile(r"""    (?:test = |    )?\(\(?"([^"]+)"(.*)""").match
    match_end = re.compile(r"    (\}\)|    \}\),)\n$").match
    first = 0
    url = ""
...
def build_test(extr, data):
    ...
    head = {
        ...
        "#category": (extr.basecategory, extr.category, extr.subcategory),
    ...
    if url := data.pop("url", None):
        instr["#sha1_url"] = url
    if metadata := data.pop("keyword", None) and isinstance(metadata, str) and len(metadata) == 40:  # noqa: F821
...
def collect_patterns():
    return {cls.pattern.pattern: cls for cls in extractor._list_classes()}
