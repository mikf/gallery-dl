...
class WeverseExtractor(Extractor):
    ...
    def items(self):
        ...
        post = self.post()
        published_at = text.parse_timestamp(
        ...
        data = {
            ...
            "hide_from_artist": post["hideFromArtist"],
            "membership_only" : post["membershipOnly"],
        }
        ...
        if post["attachment"]:
            ...
            if isinstance(self, WeverseMomentExtractor):
                ...
                self._extract_media(post, files)
    ...
    def _extract_post(self, post, files):
        ...
        for index, id in enumerate(attachment_order, 1):
            file = {
                "num": index,
            }
    ...
    def _extract_moment(self, post, files):
        moment = next(post["extension"][key]
    ...
    def _extract_media(self, post, files):
        extension = post["extension"]
        categories = [{
            "id"   : category["id"],
            "type" : category["type"],
            "title": category["title"],
        } for category in extension["mediaInfo"]["categories"]]
        data = {
            "title"     : extension["mediaInfo"]["title"],
            "media_type": extension["mediaInfo"]["mediaType"],
        ...
        if "image" in extension:
            for index, photo in enumerate(extension["image"]["photos"], 1):
                file = data.copy()
                file["num"] = index
                file.update(self._extract_image(photo))
                files.append(file)
        elif "video" in extension:
            if not self.videos:
                return
            file = data
            file.update(self._extract_video(extension["video"]))
            files.append(file)
        else:
            ...
            file = data
            file["num"] = 1
            file.update(self._extract_embed(extension["youtube"]))
            files.append(file)
...
class WeversePostExtractor(WeverseExtractor):
    ...
    subcategory = "post"
    directory_fmt = ("{category}", "{community[name]}",
                     "{author[id]}", "{post_id}")
...
class WeverseMomentExtractor(WeverseExtractor):
    ...
    subcategory = "moment"
    pattern = (BASE_PATTERN + "/moment" + MEMBER_ID_PATTERN +
...
class WeverseAPI():
    ...
    def __init__(self, extractor):
        ...
        self.access_token = cookies.get(token_cookie_name,
    ...
    def get_latest_community_media(self, community_keyword):
        community_id = self.get_community_id(community_keyword)
        endpoint = "/media/v1.0/community-{}/more".format(
            community_id)
        params = {
            "fieldSet"  : "postsV1",
            "filterType": "RECENT",
