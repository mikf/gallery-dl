...
class _4archiveThreadExtractor(Extractor):
    ...
    def items(self):
        ...
        for post in posts:
            ...
            if "url" in post:
                yield Message.Url, post["url"], text.nameext_from_url(post["filename"], post)
    ...
    def parse(post):
        ...
        data = {
            ...
            "date": text.parse_datetime(
                extr('class="dateTime postNum">', "<").strip(), "%Y-%m-%d %H:%M:%S"
            ),
        ...
        if 'class="file"' in post:
            ...
            data.update(
                {
                    "url": extr('href="', '"'),
                    "filename": extr('rel="noreferrer noopener"', "</a>").strip()[1:],
