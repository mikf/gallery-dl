...
class HentaifoundryExtractor(Extractor):
    ...
    def _parse_post(self, path):
        ...
        data = {
            ...
            "description": text.unescape(
                text.remove_html(
                    extr(">Description</div>", "</section>").replace("\r\n", "\n"), "", ""
                )
            ),
            "ratings": [
                text.unescape(r)
                for r in text.extract_iter(extr("class='ratings_box'", "</div>"), "title='", "'")
            ],
        ...
        if "<object " in body:
            ...
            data["height"] = text.parse_int(text.extr(body, "name='height' value='", "'"))
        else:
            data["src"] = text.urljoin(self.root, text.unescape(text.extr(body, 'src="', '"')))
    ...
    def _parse_story(self, html):
        ...
        data = {
            "user": self.user,
            "title": text.unescape(extr("<div class='titlebar'>", "</a>").rpartition(">")[2]),
        ...
        for c in ("Chapters", "Words", "Comments", "Views", "Rating"):
            data[c.lower()] = text.parse_int(extr(">" + c + ":</span>", "<").replace(",", ""))

        data["description"] = text.unescape(extr("class='storyDescript'>", "<div"))
        path = extr('href="', '"')
