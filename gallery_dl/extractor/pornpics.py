...
class PornpicsExtractor(Extractor):
    ...
    def _pagination(self, url, params=None):
        if params is None:
            ...
            for path in text.extract_iter(page, 'class="rel-link" href="', '"'):
        ...
        while True:
            galleries = self.request(url, params=params, headers=headers).json()
...
class PornpicsGalleryExtractor(PornpicsExtractor, GalleryExtractor):
    ...
    def images(self, page):
        return [(url, None) for url in text.extract_iter(page, "class='rel-link' href='", "'")]
