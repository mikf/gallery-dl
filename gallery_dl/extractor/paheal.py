...
class PahealExtractor(Extractor):
    ...
    def _extract_post(self, post_id):
        ...
        post = {
            ...
            "tags": extr(": ", "<"),
            "md5": extr("/_thumbs/", "/"),
            "file_url": (extr("id='main_image' src='", "'") or extr("<source src='", "'")),
            "uploader": text.unquote(extr("class='username' href='/user/", "'")),
            "date": text.parse_datetime(extr("datetime='", "'"), "%Y-%m-%dT%H:%M:%S%z"),
            "source": text.unescape(text.extr(extr(">Source Link<", "</td>"), "href='", "'")),
        }
...
class PahealTagExtractor(PahealExtractor):
    ...
    pattern = r"(?:https?://)?(?:rule34|rule63|cosplay)\.paheal\.net" r"/post/list/([^/?#]+)"
...
class PahealPostExtractor(PahealExtractor):
    ...
    subcategory = "post"
    pattern = r"(?:https?://)?(?:rule34|rule63|cosplay)\.paheal\.net" r"/post/view/(\d+)"
