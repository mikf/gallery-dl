    ...
    def items(self):
        ...
        for info in self._pagination(page):
            ...
            item = {
                ...
                "_extractor": (PixnetFolderExtractor if "/folder/" in url else PixnetSetExtractor),
            }
            item.update(data)
    ...
    def _pagination(self, page):
        while True:
            ...
            if pnext is None and 'name="albumpass">' in page:
                raise exception.StopExtraction("Album %s is password-protected.", self.item_id)
...
class PixnetSetExtractor(PixnetExtractor):
    ...
    subcategory = "set"
    url_fmt = "{}/album/set/{}"
    directory_fmt = ("{category}", "{blog}", "{folder_id} {folder_title}", "{set_id} {set_title}")
