...
class E621Extractor(danbooru.DanbooruExtractor):
    ...
    def _get_notes(self, id):
        return self.request(f"{self.root}/notes.json?search[post_id]={id}").json()

    @memcache(keyarg=1)
    def _get_pools(self, ids):
        pools = self.request(f"{self.root}/pools.json?search[id]={ids}").json()
        for pool in pools:
...
class E621PoolExtractor(E621Extractor, danbooru.DanbooruPoolExtractor):
    ...
    def posts(self):
        ...
        id_to_post = {
            post["id"]: post
            for post in self._pagination("/posts.json", {"tags": "pool:" + self.pool_id})
        }
