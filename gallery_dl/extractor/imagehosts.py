...
class ImagehostImageExtractor(Extractor):
    ...
    def __init__(self, match):
        ...
        self.page_url = "http{}://{}".format("s" if self._https else "", match.group(1))
...
class ImxtoImageExtractor(ImagehostImageExtractor):
    ...
    category = "imxto"
    pattern = r"(?:https?://)?(?:www\.)?((?:imx\.to|img\.yt)" r"/(?:i/|img-)(\w+)(\.html)?)"
...
class ImagevenueImageExtractor(ImagehostImageExtractor):
    ...
    category = "imagevenue"
    pattern = (
        r"(?:https?://)?((?:www|img\d+)\.imagevenue\.com" r"/([A-Z0-9]{8,10}|view/.*|img\.php\?.*))"
    )
...
class ImagetwistImageExtractor(ImagehostImageExtractor):
    ...
    category = "imagetwist"
    pattern = r"(?:https?://)?((?:www\.|phun\.)?" r"image(?:twist|haha)\.com/([a-z0-9]{12}))"
...
class ImagetwistGalleryExtractor(ImagehostImageExtractor):
    ...
    category = "imagetwist"
    subcategory = "gallery"
    pattern = r"(?:https?://)?((?:www\.|phun\.)?" r"image(?:twist|haha)\.com/(p/[^/?#]+/\d+))"
...
class PixhostImageExtractor(ImagehostImageExtractor):
    ...
    category = "pixhost"
    pattern = r"(?:https?://)?((?:www\.)?pixhost\.(?:to|org)" r"/show/\d+/(\d+)_[^/?#]+)"
...
class PostimgGalleryExtractor(ImagehostImageExtractor):
    ...
    subcategory = "gallery"
    pattern = (
        r"(?:https?://)?((?:www\.)?(?:postim(?:ages|g)|pixxxels)" r"\.(?:cc|org)/gallery/([^/?#]+))"
    )
