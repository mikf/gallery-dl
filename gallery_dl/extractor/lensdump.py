...
class LensdumpAlbumExtractor(LensdumpBase, GalleryExtractor):
    ...
    def metadata(self, page):
        return {
            ...
            "title": text.unescape(text.extr(page, 'property="og:title" content="', '"').strip()),
        }
    ...
    def images(self, page):
        for image in self._pagination(page, ' class="list-item ', ">"):
            data = util.json_loads(
                text.unquote(
                    text.extr(image, "data-object='", "'") or text.extr(image, 'data-object="', '"')
                )
            )
...
class LensdumpAlbumsExtractor(LensdumpBase, Extractor):
    ...
    def items(self):
        ...
        params = text.parse_query(query) if query else {"sort": "date_asc", "page": "1"}
...
class LensdumpImageExtractor(LensdumpBase, Extractor):
    ...
    def items(self):
        ...
        data = {
            ...
            "date": text.parse_datetime(extr('<span title="', '"'), "%Y-%m-%d %H:%M:%S"),
        }
