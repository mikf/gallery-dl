...
class InkbunnyFavoriteExtractor(InkbunnyExtractor):
    ...
    def metadata(self):
        ...
        url = f"{self.root}/userfavorites_process.php?favs_user_id={self.user_id}"
...
class InkbunnyUnreadExtractor(InkbunnyExtractor):
    ...
    subcategory = "unread"
    pattern = (
        BASE_PATTERN + r"/submissionsviewall\.php" r"\?((?:[^#]+&)?mode=unreadsubs(?:&[^#]+)?)"
    )
    example = "https://inkbunny.net/submissionsviewall.php" "?text=&mode=unreadsubs&type="
...
class InkbunnySearchExtractor(InkbunnyExtractor):
    ...
    subcategory = "search"
    pattern = BASE_PATTERN + r"/submissionsviewall\.php" r"\?((?:[^#]+&)?mode=search(?:&[^#]+)?)"
    example = "https://inkbunny.net/submissionsviewall.php" "?text=TAG&mode=search&type="
...
class InkbunnyFollowingExtractor(InkbunnyExtractor):
    ...
    example = "https://inkbunny.net/watchlist_process.php" "?mode=watching&user_id=12345"

    def __init__(self, match):
        InkbunnyExtractor.__init__(self, match)
        self.user_id = match.group(1) or text.parse_query(match.group(2)).get("user_id")
...
class InkbunnyAPI:
    ...
    def set_allowed_ratings(self, nudity=True, sexual=True, violence=True, strong_violence=True):
