...
BASE_PATTERN = (
    r"(?:https?://)?(?:tcb(?:-backup\.bihar-mirchi|scans)" r"|onepiecechapters)\.(?:com|me)"
)
...
class TcbscansChapterExtractor(ChapterExtractor):
    ...
    def images(self, page):
        return [
            (url, None)
            for url in text.extract_iter(page, '<img class="fixed-ratio-content" src="', '"')
        ]

    def metadata(self, page):
        manga, _, chapter = text.extr(page, 'font-bold mt-8">', "</h1>").rpartition(" - Chapter ")
...
class TcbscansMangaExtractor(MangaExtractor):
    ...
    def chapters(self, page):
        data = {
            "manga": text.unescape(text.extr(page, 'class="my-3 font-bold text-3xl">', "</h1>")),
            "lang": "en",
            "language": "English",
        }
        ...
        for chapter in text.extract_iter(page, "<a", "</a>"):
            ...
            data["title"] = text.unescape(text.extr(chapter, 'text-gray-500">', "</div>"))
            chapter = text.extr(chapter, 'font-bold">', "</div>").rpartition(" Chapter ")[2]
            chapter, sep, minor = chapter.partition(".")
