...
class KoharuExtractor(Extractor):
    ...
    def _pagination(self, endpoint, params):
        ...
        while True:
            data = self.request(url_api, params=params, headers=self.headers).json()
            ...
            for entry in entries:
                url = "{}/g/{}/{}".format(self.root, entry["id"], entry["public_key"])
...
class KoharuFavoriteExtractor(KoharuExtractor):
    ...
    def login(self):
        ...
        if username:
            self.headers["Authorization"] = "Bearer " + self._login_impl(username, password)
            return
    ...
    def _login_impl(self, username, password):
        ...
        response = self.request(url, method="POST", headers=self.headers, data=data)
