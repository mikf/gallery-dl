...
class WebtoonsEpisodeExtractor(WebtoonsBase, GalleryExtractor):
    ...
    pattern = BASE_PATTERN + r"/([^/?#]+)/([^/?#]+)/(?:[^/?#]+))" r"/viewer(?:\?([^#'\"]+))"
    example = "https://www.webtoons.com/en/GENRE/TITLE/NAME/viewer" "?title_no=123&episode_no=12345"
    test = (
        (
            (
    ...
    def __init__(self, match):
        self.path, self.lang, self.genre, self.comic, self.query = match.groups()
    ...
    def metadata(self, page):
        ...
        episode = extr(">#", "<") if extr('<span class="tx _btnOpenEpisodeList ', '"') else ""
...
class WebtoonsComicExtractor(WebtoonsBase, Extractor):
    ...
    def get_episode_urls(page):
        ...
        return [match.group(0) for match in WebtoonsEpisodeExtractor.pattern.finditer(page)]
