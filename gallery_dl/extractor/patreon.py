...
class PatreonExtractor(Extractor):
    ...
    def _init(self):
        if self.cookies_check(("session_id",)):
            self.session.headers["User-Agent"] = "Patreon/72.2.28 (Android; Android 14; Scale/2.10)"
        else:
            if self._warning:
                PatreonExtractor._warning = False
                self.log.warning("no 'session_id' cookie set")
            self.session.headers["User-Agent"] = "Patreon/7.6.28 (Android; Android 11; Scale/2.10)"
    ...
    def items(self):
        ...
        for post in self.posts():
            ...
            post["num"] = 0
            hashes = set()
            for kind, url, name in itertools.chain.from_iterable(g(post) for g in generators):
    ...
    def _process(self, post, included):
        ...
        if attr.get("current_user_can_view", True):
            ...
            attr["date"] = text.parse_datetime(attr["published_at"], "%Y-%m-%dT%H:%M:%S.%f%z")
    ...
    def _build_url(endpoint, query):
        return (
            ...
            "preview_views,video_duration" + query + "&json-api-version=1.0"
        )
    ...
    def _extract_bootstrap(self, page):
        data = text.extr(page, 'id="__NEXT_DATA__" type="application/json">', "</script")
        if data:
            try:
        ...
        bootstrap = text.extr(page, 'window.patreon = wrapInProxy({"bootstrap":', '},"apiServer"')
        if bootstrap:
            return util.json_loads(bootstrap + "}")
...
class PatreonCreatorExtractor(PatreonExtractor):
    ...
    def posts(self):
        ...
        url = self._build_url(
            "posts",
            (
                "&filter[campaign_id]=" + campaign_id + "&filter[contains_exclusive_posts]=true"
                "&filter[is_draft]=false" + filters + "&sort=" + query.get("sort", "-published_at")
            ),
        )
    ...
    def _get_campaign_id(self, creator, query):
        ...
        user_id = query.get("u")
        url = f"{self.root}/user?u={user_id}" if user_id else f"{self.root}/{creator}"
        ...
        try:
            ...
            raise exception.StopExtraction(
                "Unable to extract campaign ID (%s: %s)", exc.__class__.__name__, exc
            )
