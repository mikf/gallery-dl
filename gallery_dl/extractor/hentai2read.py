...
class Hentai2readChapterExtractor(Hentai2readBase, ChapterExtractor):
    ...
    def metadata(self, page):
        ...
        match = re.match(
            r"Reading (.+) \(([^)]+)\) Hentai(?: by (.+))? - " r"([^:]+): (.+) . Page 1 ", title
        )
        return {
            "manga": match.group(1),
    ...
    def images(self, page):
        ...
        return [("https://hentaicdn.com/hentai" + part, None) for part in util.json_loads(images)]
...
class Hentai2readMangaExtractor(Hentai2readBase, MangaExtractor):
    ...
    def chapters(self, page):
        ...
        mtype, pos = text.extract(page, '<small class="text-danger">[', "]</small>", pos)
