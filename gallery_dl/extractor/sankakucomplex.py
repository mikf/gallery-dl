...
class SankakucomplexArticleExtractor(SankakucomplexExtractor):
    ...
    pattern = r"(?:https?://)?(?:news|www)\.sankakucomplex\.com" r"/(\d\d\d\d/\d\d/\d\d/[^/?#]+)"
    ...
    def items(self):
        ...
        data = {
            ...
            "date": text.parse_datetime(extr('property="article:published_time" content="', '"')),
        }
    ...
    def _extract_embeds(content):
        return ["ytdl:" + url for url in re.findall(r"<iframe [^>]*src=[\"']([^\"']+)", content)]
...
class SankakucomplexTagExtractor(SankakucomplexExtractor):
    ...
    subcategory = "tag"
    pattern = (
        r"(?:https?://)?(?:news|www)\.sankakucomplex\.com" r"/((?:tag|category|author)/[^/?#]+)"
    )
    ...
    def items(self):
        ...
        while True:
            ...
            if response.status_code >= 400:
                return
            for url in util.unique_sequence(text.extract_iter(response.text, 'data-direct="', '"')):
