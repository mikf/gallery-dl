...
class Ao3WorkExtractor(Ao3Extractor):
    ...
    def items(self):
        ...
        if response.url.endswith("/users/login?restricted=true"):
            raise exception.AuthorizationError("Login required to access member-only works")
        page = response.text
        if len(page) < 20000 and '<h2 class="landmark heading">Adult Content Warning</' in page:
        ...
        data = {
            ...
            "relationships": text.split_html(extr('<dd class="relationship tags">', "</dd>")),
            ...
            "date": text.parse_datetime(extr('<dd class="published">', "<"), "%Y-%m-%d"),
            ...
            "chapters": chapters,
            "comments": text.parse_int(extr('<dd class="comments">', "<").replace(",", "")),
            ...
            "author": text.unescape(text.remove_html(extr(' class="byline heading">', "</h3>"))),
            "summary": text.split_html(extr(' class="heading">Summary:</h3>', "</div>")),
        }
...
class Ao3UserExtractor(Ao3Extractor):
    ...
    subcategory = "user"
    pattern = BASE_PATTERN + r"/users/([^/?#]+(?:/pseuds/[^/?#]+)?)" r"(?:/profile)?/?(?:$|\?|#)"
...
class Ao3UserWorksExtractor(Ao3Extractor):
    ...
    pattern = BASE_PATTERN + r"(/users/([^/?#]+)/(?:pseuds/([^/?#]+)/)?" r"works(?:/?\?.+)?)"
...
class Ao3UserSeriesExtractor(Ao3Extractor):
    ...
    pattern = BASE_PATTERN + r"(/users/([^/?#]+)/(?:pseuds/([^/?#]+)/)?" r"series(?:/?\?.+)?)"
...
class Ao3UserBookmarkExtractor(Ao3Extractor):
    ...
    pattern = BASE_PATTERN + r"(/users/([^/?#]+)/(?:pseuds/([^/?#]+)/)?" r"bookmarks(?:/?\?.+)?)"
