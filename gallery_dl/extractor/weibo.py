...
class WeiboExtractor(Extractor):
        ...
        if response.history:
            if "login.sina.com" in response.url:
                raise exception.StopExtraction(
                    "HTTP redirect to login page (%s)", response.url.partition("?")[0]
                )
    ...
    def items(self):
        ...
        for status in self.statuses():
            ...
            status["date"] = text.parse_datetime(status["created_at"], "%a %b %d %H:%M:%S %z %Y")
    ...
    def _user_id(self):
        ...
        url = "{}/ajax/profile/info?{}={}".format(
            self.root, "screen_name" if self._prefix == "n" else "custom", self.user
        )
        ...
        while True:
            ...
            if not data.get("ok"):
                ...
                if "since_id" not in params:  # first iteration
                    raise exception.StopExtraction('"%s"', data.get("msg") or "unknown error")
    ...
    def _sina_visitor_system(self, response):
        ...
        page = Extractor.request(self, passport_url, method="POST", headers=headers, data=data).text
