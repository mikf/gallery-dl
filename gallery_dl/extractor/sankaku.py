...
class SankakuArticleExtractor(Extractor):
    """Extractor for articles on www.sankakucomplex.com"""
    category = "sankaku"
    subcategory = "article"
    directory_fmt = ("{category}", "Articles", "{date:%Y-%m-%d} {title}")
    filename_fmt = "{filename}.{extension}"
    archive_fmt = "a_{date:%Y%m%d}_{filename}"
    ...
    test = (
        ...
        ("https://www.sankakucomplex.com/2009/12/01/sexy-goddesses-of-2ch", {
            ...
            "keyword": "a7876de642bf3e68fb4743dcd4d4e8778f2c17ab",
        }),
    )
    ...
    def items(self):
        url = "{}/{}/{}/?pg=X".format(self.root, self.date, self.title)
