...
class KemonopartyExtractor(Extractor):
    ...
    def _init(self):
        ...
        self._json_dumps = json.JSONEncoder(
            ensure_ascii=False, check_circular=False, sort_keys=True, separators=(",", ":")
        ).encode
    ...
    def items(self):
        ...
        if self.config("metadata"):
            username = text.unescape(
                text.extract(
                    self.request(self.user_url).text, '<meta name="artist_name" content="', '"'
                )[0]
            )
        if self.config("dms"):
            dms = True
        ...
        for post in posts:
            ...
            post["date"] = self._parse_datetime(post.get("published") or post.get("added") or "")
    ...
    def login(self):
        ...
        if username:
            self.cookies_update(self._login_impl((username, self.cookies_domain), password))
    ...
    def _login_impl(self, username, password):
        ...
        if (
            response.url.endswith("/account/login")
            and "Username or password is incorrect" in response.text
        ):
    ...
    def _extract_comments(self, post):
        url = "{}/{}/user/{}/post/{}".format(self.root, post["service"], post["user"], post["id"])
            ...
            comments.append(
                {
                    ...
                    "body": extr('<section class="comment__body">', "</section>").strip(),
                    "date": extr('datetime="', '"'),
                }
            )
        return comments
    ...
    def _extract_cards(self, post, type):
        url = "{}/{}/user/{}/{}".format(self.root, post["service"], post["user"], type)
        page = self.request(url).text
    ...
    def _discord_channels(self, server):
        url = f"{self.root}/api/v1/discord/channel/lookup/{server}"
...
class KemonopartyUserExtractor(KemonopartyExtractor):
    ...
    def __init__(self, match):
        ...
        self.api_url = f"{self.root}/api/v1/{service}/user/{user_id}"
...
class KemonopartyPostExtractor(KemonopartyExtractor):
    ...
    def __init__(self, match):
        _, _, service, user_id, post_id, self.revision, self.revision_id = match.groups()
        self.subcategory = service
        KemonopartyExtractor.__init__(self, match)
        self.api_url = f"{self.root}/api/v1/{service}/user/{user_id}/post/{post_id}"
        self.user_url = f"{self.root}/{service}/user/{user_id}"
...
class KemonopartyDiscordExtractor(KemonopartyExtractor):
    ...
    def items(self):
        ...
        if self.channel_id:
            ...
            key = "id" if self.channel.isdecimal() and len(self.channel) >= 16 else "name"
    ...
    def posts(self):
        url = f"{self.root}/api/v1/discord/channel/{self.channel_id}"
        params = {"o": 0}
...
class KemonopartyFavoriteExtractor(KemonopartyExtractor):
    ...
    def __init__(self, match):
        ...
        self.favorites = self.params.get("type") or self.config("favorites") or "artist"
    ...
    def items(self):
        ...
        if self.favorites == "artist":
            users = self.request(self.root + "/api/v1/account/favorites?type=artist").json()
            ...
            for user in users:
                ...
                url = "{}/{}/user/{}".format(self.root, user["service"], user["id"])
                yield Message.Queue, url, user

        elif self.favorites == "post":
            posts = self.request(self.root + "/api/v1/account/favorites?type=post").json()
