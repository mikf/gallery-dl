...
def parse_command_line(module, argv):
    ...
    if getattr(opts, "wait_for_video", None) is not None:
        ...
        opts.wait_for_video = (module.parse_duration(min_wait), module.parse_duration(max_wait))
    ...
    if ytdlp:
        ...
        if isinstance(parse_metadata, dict):
            ...
            opts.parse_metadata = list(
                itertools.chain.from_iterable(map(metadataparser_actions, parse_metadata))
            )
    ...
    match_filter = (
        None if opts.match_filter is None else module.match_filter_func(opts.match_filter)
    )
    ...
    return {
        ...
        "force_write_download_archive": getattr(opts, "force_write_download_archive", None),
        ...
        "concurrent_fragment_downloads": getattr(opts, "concurrent_fragment_downloads", None),
        ...
        "youtube_include_dash_manifest": getattr(opts, "youtube_include_dash_manifest", None),
        "youtube_include_hls_manifest": getattr(opts, "youtube_include_hls_manifest", None),
...
def legacy_postprocessors(opts, module, ytdlp, compat_opts):
    ...
    if opts.embedsubtitles:
        ...
        if ytdlp:
            pp["already_have_subtitle"] = opts.writesubtitles and "no-keep-subs" not in compat_opts
        postprocessors.append(pp)
    ...
    if opts.embedthumbnail:
        already_have_thumbnail = opts.writethumbnail or getattr(opts, "write_all_thumbnails", False)
        postprocessors.append(
            {
                "key": "EmbedThumbnail",
