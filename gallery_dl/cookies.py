...
SUPPORTED_BROWSERS_CHROMIUM = {"brave", "chrome", "chromium", "edge", "opera", "thorium", "vivaldi"}
...
def load_cookies(browser_specification):
    ...
    if browser_name in SUPPORTED_BROWSERS_CHROMIUM:
...
def load_cookies_firefox(profile=None, container=None, domain=None):
    ...
    with DatabaseConnection(path) as db:
        cookies = [
            ...
            for name, value, domain, path, secure, expires in db.execute(sql, parameters)
        ]
...
def load_cookies_chromium(browser_name, profile=None, keyring=None, domain=None):
    ...
    with DatabaseConnection(path) as db:
        ...
        try:
            ...
            _log_warning(
                "Failed to get cookie database meta version (%s: %s)", exc.__class__.__name__, exc
            )
            meta_version = 0
        ...
        for domain, name, value, enc_value, path, expires, secure in rows:
            ...
            expires = int(expires) // 1000000 - 11644473600 if expires else None
    ...
    _log_info(
        "Extracted %s cookies from %s%s", len(cookies), browser_name.capitalize(), failed_message
    )
...
def _firefox_cookies_database(profile=None, container=None):
    ...
    if path is None:
        raise FileNotFoundError("Unable to find Firefox cookies database in " f"{search_root}")
    _log_debug("Extracting cookies from %s", path)
    ...
    if not container or container == "none":
        ...
        try:
            ...
            _log_error("Unable to read Firefox container database at '%s'", containers_path)
            raise
        except KeyError:
            identities = ()
        ...
        _log_debug("Only loading cookies from container '%s' (ID %s)", container, container_id)
...
def _safari_parse_cookies_page(data, cookies, domain=None):
    ...
    for i, record_offset in enumerate(record_offsets):
        ...
        record_length = _safari_parse_cookies_record(data[record_offset:], cookies, domain)
        ...
        if host:
            if host[0] == ".":
                if host[1:] != domain and not domain.endswith(host):
                    return record_size
            elif host != domain and ("." + host) != domain:
                return record_size
...
def _chromium_cookies_database(profile, config):
    ...
    if path is None:
        raise FileNotFoundError(
            "Unable to find {} cookies database in " "'{}'".format(config["browser"], search_root)
        )
    return path
...
def _chromium_cookie_decryptor(browser_root, browser_keyring_name, keyring=None, meta_version=0):
    if sys.platform in ("win32", "cygwin"):
...
def _get_kwallet_network_wallet():
    ...
    try:
        ...
        _log_warning("Error while obtaining NetworkWallet (%s: %s)", exc.__class__.__name__, exc)
        return default_wallet
...
def _get_kwallet_password(browser_keyring_name):
    ...
    try:
        ...
        if proc.returncode != 0:
            _log_error(
                f"kwallet-query failed with return code {proc.returncode}. "
        ...
        if stdout.lower().startswith(b"failed to read"):
            _log_debug("Failed to read password from kwallet. " "Using empty string instead")
        ...
        _log_warning("Error when running kwallet-query (%s: %s)", exc.__class__.__name__, exc)
        return b""
...
def _get_linux_keyring_password(browser_keyring_name, keyring):
    ...
    if keyring == KEYRING_GNOMEKEYRING:
        return _get_gnome_keyring_password(browser_keyring_name)
    if keyring == KEYRING_BASICTEXT:
...
def _get_mac_keyring_password(browser_keyring_name):
    ...
    try:
        ...
        _log_warning("Error when using find-generic-password (%s: %s)", exc.__class__.__name__, exc)
        return None
...
class DatabaseConnection:
    ...
    def __enter__(self):
        try:
            ...
            _log_debug(
                "Falling back to temporary database copy (%s: %s)", exc.__class__.__name__, exc
            )
...
def _decrypt_aes_cbc(ciphertext, key, offset=0, initialization_vector=b" " * 16):
    plaintext = aes.unpad_pkcs7(aes.aes_cbc_decrypt_bytes(ciphertext, key, initialization_vector))
    if offset:
...
def _decrypt_aes_gcm(ciphertext, key, nonce, authentication_tag, offset=0):
    try:
        plaintext = aes.aes_gcm_decrypt_and_verify_bytes(ciphertext, key, authentication_tag, nonce)
        if offset:
            plaintext = plaintext[offset:]
        return plaintext.decode()
    except UnicodeDecodeError:
...
def _parse_browser_specification(browser, profile=None, keyring=None, container=None, domain=None):
