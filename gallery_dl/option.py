...
class UgoiraAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        value = self.const if self.const else value.strip().lower()
...
class PrintAction(argparse.Action):
    def __call__(self, parser, namespace, value, option_string=None):
        ...
        if "{" not in format_string and " " not in format_string and format_string[0] != "\f":
...
def build_parser():
    ...
    general.add_argument(
        ...
        dest="filename",
        metavar="FORMAT",
        help=("Filename format string for downloaded files " "('/O' for \"original\" filenames)"),
    )
    general.add_argument(
        "-d",
        "--destination",
    ...
    general.add_argument(
        ...
        dest="clear_cache",
        metavar="MODULE",
        help="Delete cached login sessions, cookies, etc. for MODULE " "(ALL to delete everything)",
    )
    ...
    output.add_argument(
        ...
        help=("Print a list of available keywords and example values " "for the given URLs"),
    )
    output.add_argument(
        "-e",
        "--error-file",
    ...
    downloader.add_argument(
        ...
        metavar="SECONDS",
        action=ConfigAction,
        help=("Number of seconds to wait between HTTP requests " "during data extraction"),
    )
    downloader.add_argument(
        "--sleep-extractor",
        ...
        metavar="SECONDS",
        action=ConfigAction,
        help=("Number of seconds to wait before starting data extraction " "for an input URL"),
    )
    downloader.add_argument(
        "--no-part",
        dest="part",
    ...
    selection.add_argument(
        ...
        metavar="N",
        type=int,
        help=("Stop current extractor run " "after N consecutive file downloads were skipped"),
    )
    selection.add_argument(
        "-T",
        "--terminate",
        ...
        type=int,
        help=(
            "Stop current and parent extractor runs "
    ...
    selection.add_argument(
        ...
        action=ConfigAction,
        help=(
            "Record all downloaded or skipped files in FILE and "
    ...
    selection.add_argument(
        ...
        metavar="RANGE",
        action=ConfigAction,
        help=("Like '--range', but applies to manga chapters " "and other delegated URLs"),
    )
    selection.add_argument(
        "--filter",
        dest="image-filter",
    ...
    selection.add_argument(
        ...
        metavar="EXPR",
        action=ConfigAction,
        help=("Like '--filter', but applies to manga chapters " "and other delegated URLs"),
    )
    ...
    postprocessor.add_argument(
        ...
        action=RenameAction,
        const=0,
        help=("Rename previously downloaded files from FORMAT " "to the current filename format"),
    )
    postprocessor.add_argument(
        "--rename-to",
        dest="postprocessors",
        ...
        action=RenameAction,
        const=1,
        help=("Rename previously downloaded files from the current filename " "format to FORMAT"),
    )
    postprocessor.add_argument(
        "--ugoira",
        dest="postprocessors",
